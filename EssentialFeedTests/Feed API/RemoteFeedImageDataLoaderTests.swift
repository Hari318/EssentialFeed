...
class RemoteFeedImageDataLoader {
    ...
    func loadImageData(from url: URL, completion: @escaping (FeedImageDataLoader.Result) -> Void) -> FeedImageDataLoaderTask {
        return HTTPTaskWrapper(wrapper: client.get(from: url) { [weak self] result in
...
class RemoteFeedImageDataLoaderTests: XCTestCase {
    ...
    func test_loadImageDataFromURL_deliversInvalidDataErrorOn200HTTPResponseWithEmptyData() {
        ...
        expect(sut, toCompleteWith: failure(error: .invalidData)) {
            let data = Data()
            client.complete(statusCode: 200, data: data)
        }
    }
    ...
    func test_cancelLoadImageDataURLTask_cancelsClientURLRequest() {
        ...
        task.cancel()
        XCTAssertEqual(client.cancelledURLs, [url], "Expected cancelled URL requests after task is cancelled")
    }
